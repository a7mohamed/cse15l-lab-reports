...
These commands that I wrote print out some useful information. For example, ls -a command printed out all of the files that were in remote server account. The command cd stands for "change directory" and used to change directories, which means it moves the command prompt into a different folder. And the cat command, which is short for "concatenate" displays the contents of one or more files without having to open the file for editing. And there are plenty more commands as well. Pretty cool huh?
